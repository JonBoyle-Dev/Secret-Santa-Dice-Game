export default function KidsGrinchOverlay({ targetTeamName, cardText, onDismiss }) {
  return (
    <div className="fixed inset-0 z-50 bg-black flex flex-col items-center justify-center p-6 text-center animate-flicker">
      ...
      {targetTeamName ? (
        <>
          <p className="font-display text-2xl sm:text-3xl text-neon-green mb-2">
            {targetTeamName} still has their own gift!
          </p>
          <p className="text-white text-lg max-w-sm mb-8">Draws: {cardText}</p>
        </>
      ) : (
