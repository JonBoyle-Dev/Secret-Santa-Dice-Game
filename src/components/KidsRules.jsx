import { KIDS_DICE_ACTIONS } from '../data/kidsDiceTable'
import { FINAL_GOES } from '../data/diceTable'

export default function KidsRules({ setScreen }) {
  return (
    <div className="min-h-screen flex flex-col items-center px-4 py-8">
      ...
      <div className="w-full max-w-sm bg-white/5 border border-neon-green/30 rounded-2xl p-4 mb-4 grunge-overlay">
        ...
        <div className="space-y-3">
          {Object.entries(KIDS_DICE_ACTIONS).map(([roll, action]) => (
      ...
      <div className="w-full max-w-sm bg-white/5 border border-neon-pink/30 rounded-2xl p-4 mb-4 grunge-overlay">
        ...
        <p className="text-white/60 text-sm mb-2">
          ...
          everyone joins in on a Group Task together.
        </p>
        <p className="text-white/60 text-sm">
          🖤 <span className="text-white">The Grinch</span> — a rare chance where whichever player is
          still holding the gift they originally brought draws a Challenge Card.
        </p>
      </div>
